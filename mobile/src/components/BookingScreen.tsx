...
export default function BookingScreen() {
    ...
    const fetchBusinesses = async () => {
      try {
        const res = await apiClient(
          "/query/v1/organizationService?guest&limit=50&~isActive=true",
        );
        if (res.ok) {
      ...
      try {
        ...
        if (timeMatch) {
          ...
          startDateTime = new Date(
            `${selectedDate}T${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")}:00`,
          );
        ...
        if (response.ok) {
          ...
          const appointmentData =
            data.appointments?.[0] || data.appointment?.[0] || {};

          addAppointment({
            id:
              appointmentData.id || Math.random().toString(36).substring(2, 9),
            ...
            progressSteps: getStepsByCategory(
  ...
  const filteredBusinesses = selectedCategory
    ? businessesData.filter(
        (b) => b.category === selectedCategory || selectedCategory === "all",
      )
    : businessesData;
  ...
  return (
    <View style={tw`flex-1 bg-white`}>
      {/* Header */}
      <View
        style={tw`px-6 py-4 flex-row items-center gap-4 border-b border-gray-100`}
      >
        <TouchableOpacity
          onPress={() => {
            reset();
      ...
      >
        ...
        <View style={tw`px-6 py-6`}>
          ...
          <ScrollView
            ...
            contentContainerStyle={tw`pb-2 gap-3`}
          >
            {CATEGORIES.map((cat) => {
              const Icon = cat.icon;
              ...
              return (
                ...
                >
                  ...
                  <Text
                    style={tw`text-xs font-bold ${isSelected ? "text-white" : "text-gray-500"}`}
                  >
                    {cat.name}
                  </Text>
                </TouchableOpacity>
        ...
        {!selectedBusiness ? (
          <View style={tw`px-6 gap-4`}>
            <View style={tw`flex-row justify-between items-center`}>
              <Text
                style={tw`text-[10px] font-bold text-gray-400 uppercase tracking-widest`}
              >
                Select a provider
              </Text>
              <Text style={tw`text-[10px] font-bold text-blue-600 uppercase`}>
              ...
              {filteredBusinesses.map((biz) => (
                ...
                >
                  <View style={tw`relative`}>
                    ...
                    />
                    <View
                      style={tw`absolute -top-1.5 -right-1.5 bg-white shadow-md border border-yellow-50 p-1.5 rounded-full`}
                    >
                  ...
                  <View style={tw`flex-1`}>
                    ...
                    <View style={tw`flex-row items-center justify-between`}>
                      <View style={tw`bg-blue-50 px-2 py-1 rounded-lg`}>
                        <Text style={tw`text-[11px] font-bold text-blue-700`}>
                          Starting {biz.services[0].price} ETB
                        </Text>
                      </View>
                      <View style={tw`flex-row items-center gap-0.5`}>
                        <Text style={tw`text-yellow-600 text-[10px] font-bold`}>
                          {biz.rating}
                        </Text>
                        <Text
                          style={tw`text-gray-400 text-[10px] font-medium ml-1`}
                        >
                          ★
                        </Text>
                      </View>
                  ...
                  <View
                    style={tw`w-10 h-10 rounded-2xl bg-gray-50 items-center justify-center`}
                  >
          ...
          <View>
            ...
            <View style={tw`px-6 mb-8`}>
              <View
                style={tw`p-4 bg-blue-50 rounded-3xl border border-blue-100 flex-row items-center justify-between`}
              >
                <View style={tw`flex-row items-center gap-3 flex-1`}>
                  <Image
                    source={
                      typeof selectedBusiness.image === "string"
                        ? { uri: selectedBusiness.image }
                  ...
                  <View style={tw`flex-1`}>
                    <Text
                      style={tw`font-bold text-blue-900 text-sm`}
                      numberOfLines={1}
                    >
                      {selectedBusiness.name}
                    </Text>
                    <Text
                      style={tw`text-[10px] font-bold text-blue-500 uppercase`}
                    >
            ...
            <View style={tw`px-6 mb-8`}>
              <View
                style={tw`p-6 bg-white rounded-[32px] border border-gray-100 shadow-sm`}
              >
                <Text
                  style={tw`text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4`}
                >
                  Select Service
                </Text>
                <View style={tw`gap-4`}>
                  {selectedBusiness.services.map((svc) => {
                    ...
                    return (
                      ...
                      >
                        <View
                          style={tw`flex-row items-start justify-between w-full`}
                        >
                          <View style={tw`flex-1 pr-4`}>
                            <Text
                              style={tw`font-bold text-base mb-1 ${isSelected ? "text-blue-900" : "text-gray-800"}`}
                              numberOfLines={1}
                            >
                              {svc.name}
                            </Text>
                            <Text
                              style={tw`text-[10px] leading-relaxed text-gray-400`}
                              numberOfLines={2}
                            >
                              {svc.description}
                            </Text>
                          ...
                          <View
                            style={tw`w-8 h-8 rounded-full border-2 items-center justify-center ${isSelected ? "bg-blue-600 border-blue-600" : "bg-gray-50 border-gray-100"}`}
                          >
                            {isSelected ? (
                              <Check size={16} color="white" strokeWidth={3} />
                            ) : (
                              <View
                                style={tw`w-1.5 h-1.5 rounded-full bg-gray-200`}
                              />
                            )}
                          </View>
                        </View>
                        ...
                        <View
                          style={tw`flex-row items-center justify-between w-full pt-1 border-t border-gray-50`}
                        >
                          <View style={tw`flex-row items-center gap-2`}>
                            <View
                              style={tw`flex-row items-center gap-1.5 px-2.5 py-1 rounded-lg bg-gray-100`}
                            >
                              {/* <Clock color="#3B82F6" size={12} />
                              <Text
                                style={tw`text-[10px] font-bold text-gray-500`}
                              >
                                {svc.duration}
                              </Text> */}
                            </View>
                            {isSelected && (
                              <View
                                style={tw`bg-blue-100 px-1.5 py-0.5 rounded`}
                              >
                                <Text
                                  style={tw`text-[9px] font-bold text-blue-600 uppercase tracking-tighter`}
                                >
                                  Selected
                                </Text>
                              </View>
                          ...
                          <Text
                            style={tw`text-sm font-bold ${isSelected ? "text-blue-600" : "text-gray-900"}`}
                          >
                            {svc.price}{" "}
                            <Text style={tw`text-[10px] font-bold opacity-60`}>
                              ETB
                            </Text>
                          </Text>
                        </View>
            ...
            <View style={tw`px-6 mb-12`}>
              <View
                style={tw`p-6 bg-gray-50 rounded-[32px] border border-gray-100 shadow-sm`}
              >
                <View style={tw`mb-6`}>
                  <View style={tw`flex-row items-center gap-2 mb-4`}>
                    <View
                      style={tw`w-5 h-5 rounded-lg bg-blue-100 items-center justify-center`}
                    >
                  ...
                  >
                    {dates.map((d) => (
                      ...
                      >
                        <Text
                          style={tw`text-[10px] font-bold uppercase opacity-80 ${selectedDate === d.value ? "text-white" : "text-gray-500"}`}
                        >
                          {d.label.split(",")[0]}
                        </Text>
                        <Text
                          style={tw`text-sm font-bold ${selectedDate === d.value ? "text-white" : "text-gray-900"}`}
                        >
                          {d.label.includes(",")
                ...
                <View>
                  ...
                  >
                    {(selectedBusiness.availableTimes || times).map((t) => (
                      ...
                      >
                        <Text
                          style={tw`text-[11px] font-bold ${selectedTime === t ? "text-white" : "text-gray-500"}`}
                        >
                          {t}
                        </Text>
                      </TouchableOpacity>
      ...
      {selectedService ? (
        <View style={tw`absolute bottom-8 left-6 right-6`}>
          <View
            style={tw`bg-gray-900 p-6 rounded-[32px] shadow-2xl shadow-blue-300 border border-gray-800`}
          >
            <View style={tw`flex-row justify-between items-center mb-5 px-1`}>
              <View>
                <Text
                  style={tw`text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1`}
                >
                  Estimated Total
                </Text>
                <Text style={tw`text-2xl font-black text-blue-400`}>
                  {selectedService.price}{" "}
                  <Text style={tw`text-xs font-bold text-blue-500`}>ETB</Text>
                </Text>
              </View>
              <View style={tw`items-end`}>
                ...
                <Text
                  style={tw`text-xs font-bold text-white`}
                  numberOfLines={1}
                >
                  {selectedService.name}
                </Text>
