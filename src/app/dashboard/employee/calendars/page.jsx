...
export default function CalendarsPage() {
  ...
  useEffect(() => {
    RequestHandler.Get("/api/v1/employee").then(async (res) => {
      if (res.ok) {
        ...
        setActiveCalendarId(employee.calendarId);
      }
    });
  }, []);
  ...
  const fetchCalendars = useCallback(async () => {
    ...
    const [countRes, dataRes] = await Promise.all([
      RequestHandler.Get(`/api/v1/employee/${employeeId}/calendars/count`),
      RequestHandler.Get(
        `/api/v1/employee/${employeeId}/calendars?${params.toString()}`,
      ),
    ]);
  ...
  const handleAddCalendar = async () => {
    ...
    const res = await RequestHandler.Post(
      `/api/v1/employee/${employeeId}/calendar`,
      {
        body: {
          name: formData.name,
  ...
  const handleEditCalendar = async () => {
    ...
    const res = await RequestHandler.Patch(
      `/api/v1/employee/${employeeId}/calendar/${selectedCalendar.id}`,
      {
        body: {
          name: formData.name,
          description: formData.description,
          ...
          <Table className=" font-mono">
            <TableHeader className="bg-muted/30 uppercase">
              <TableRow>
                ...
                <TableHead className="font-bold text-center">Main</TableHead>
                ...
                <TableRow key={calendar.id}>
                  ...
                  <TableCell>
                    <div className="font-semibold text-primary">
                      <Link
                        href={"/dashboard/employee/calendar/" + calendar.id}
                      >
                        {calendar.name}
                      </Link>
                    </div>
                  ...
                  <TableCell className="flex justify-center">
                    <Checkbox
                      ...
                      onCheckedChange={() => {
                        handleSetDefaultCalendar(calendar.id);
            ...
            >
              <CardHeader className="pb-3 border-b bg-muted/5 flex flex-row items-center justify-between space-y-0">
                <CardTitle className="font-bold text-primary truncate pr-4 flex justify-between gap-1 w-full">
                  <Link href={"/dashboard/employee/calendar/" + calendar.id}>
                    {calendar.name}
                  </Link>
                  <Badge
                    variant={
                      activeCalendarId === calendar.id ? "default" : "outline"
                    }
                    onClick={() => {
                      handleSetDefaultCalendar(calendar.id);
                      setActiveCalendarId((prev) =>
