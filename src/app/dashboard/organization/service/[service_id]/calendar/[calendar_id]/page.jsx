...
export default function SmartCalendarGrid() {
  const { service_id, calendar_id } = useParams();
  ...
  // --- Data Fetching ---
  // useEffect(() => {
  //   RequestHandler.Get(`/api/v1/service/${service_id}/calendar/${calendar_id}`).then(async (res) => {
  //     if (res.ok) {
  //       const { service } = await res.json();
  //       setServiceId(service.userId);
  //     }
  //   });
  // }, []);
  ...
  const fetchCalendars = useCallback(async () => {
    ...
    const [dataRes] = await Promise.all([
      RequestHandler.Get(
        `/api/v1/service/${service_id}/calendar/${calendar_id}?${params.toString()}`,
      ),
    ]);
