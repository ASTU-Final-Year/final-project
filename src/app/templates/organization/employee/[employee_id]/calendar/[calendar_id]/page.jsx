...
export default function SmartCalendarGrid() {
  const { employee_id, calendar_id } = useParams();
  ...
  // --- Data Fetching ---
  // useEffect(() => {
  //   RequestHandler.Get(`/api/v1/employee/${employee_id}/calendar/${calendar_id}`).then(async (res) => {
  //     if (res.ok) {
  ...
  const fetchCalendars = useCallback(async () => {
    ...
    const [dataRes] = await Promise.all([
      RequestHandler.Get(
        `/api/v1/employee/${employee_id}/calendar/${calendar_id}?${params.toString()}`,
      ),
    ]);
